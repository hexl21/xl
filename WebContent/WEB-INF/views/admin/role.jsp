...
<title>角色管理</title>
				...
				formatter : function(value, row, index) {
					var str = '&nbsp;';
						if ($.canGrant) {
							...
							str += $.formatString('<a href="javascript:void(0);" onclick="grantFun(\'{0}\');" class="easyui-linkbutton" data-options="plain:true,iconCls:icon_add">授权</a>', row.id);
						}
					if(row.isdefault!=0){
						str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
						if ($.canEdit) {
			...
			buttons : [ {
				...
				handler : function() {
					...
					$.messager.show({
		    			title:'提示',
		    			timeout:3000,
		    			msg:'<div class="light-info"><div class="light-tip icon-tip"></div><div>执行成功</div></div>',
		    			showType:'show'
		    		});
				}
			} ]
	...
	function  append(){
        var node = $('#dataGrid').tree('getSelected');
            $('#tree').tree('append',{
