package xl.sys.resource.domain.impl;
...
import xl.sys.resource.model.Tresource;
...
public class ResourceServiceImpl implements ResourceServiceI {
	...
	public List<Tree> tree(SessionInfo sessionInfo) {
		...
		if (sessionInfo != null) {
			if("admin".equals(sessionInfo.getLoginname())){
				...
				params.put("userId", String.valueOf(sessionInfo.getId()));// 自查自己有权限的资源
				l = resourceDao
	...
	public List<Tree> allTree(boolean flag) {
		...
		if(flag){
			...
			l = resourceDao.find("select distinct t from Tresource t left join fetch t.resource where t.resourcetype =0 order by t.seq");
		}
		if ((l != null) && (l.size() > 0)) {
			for (Tresource r : l) {
				Tree tree = new Tree();
	...
	public List<String> resourceAllList() {
		...
		List<Tresource> l = resourceDao.find("select distinct t from Tresource t left join fetch t.resource  order by t.seq");
